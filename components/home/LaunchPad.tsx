...
export function LaunchPad({ lang, t }: Props) {
  return (
    <section id="launchpad" className="bg-opc-dark py-16 md:py-24 px-5 md:px-6 border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        <h2 className="font-serif text-3xl md:text-4xl font-black text-white mb-2 md:mb-3 tracking-tight">{t.heading}</h2>
        <p className="font-sans text-sm md:text-base text-white/50 mb-8 md:mb-12">{t.sub}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
          {LAUNCHPAD_FEATURED.map((deal) => {
            ...
            return (
              <div key={deal.id} className="bg-white/5 border border-white/8 rounded-xl p-5 md:p-6 flex flex-col">
                ...
                <h3 className="font-serif text-base md:text-lg font-bold text-white mb-2 leading-snug">{title}</h3>
                <p className="font-sans text-sm text-white/55 leading-relaxed flex-1 mb-4 md:mb-5">{desc}</p>
