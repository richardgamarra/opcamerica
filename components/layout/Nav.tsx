      ...
      <div className="flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-8 font-sans text-sm text-white/70">
          {navLinks.map(({ key, href }) => (
            <Link key={key} href={href} className="transition-colors hover:text-white">
              {t[key]}
            </Link>
          ))}
        </div>
        ...
        <div className="hidden md:flex gap-3 items-center font-sans text-sm">
          <button
            onClick={switchLang}
            className="border border-white/30 text-white/75 hover:text-white px-4 py-1.5 rounded-full text-xs transition-colors"
          >
            🌐 {t.langToggle}
          </button>
          <Link href={`/${lang}/auth/signin`} className="text-white/70 hover:text-white transition-colors">
            {t.signIn}
          </Link>
          <Link
        ...
        <div className="flex md:hidden items-center gap-2">
          <button
            onClick={switchLang}
            className="border border-white/30 text-white/75 px-3 py-1 rounded-full text-xs"
          >
            🌐 {lang.toUpperCase()}
          </button>
          <button
      ...
      {menuOpen && (
        <div className="md:hidden mt-4 pb-4 border-t border-white/10 pt-4 flex flex-col gap-4">
          ...
          <div className="flex gap-3 mt-2">
            <Link href={`/${lang}/auth/signin`} className="font-sans text-sm text-white/60">
              {t.signIn}
            </Link>
            <Link
              href={`/${lang}/auth/signup`}
