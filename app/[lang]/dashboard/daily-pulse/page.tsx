...
function TypeIcon({ type }: { type: string }) {
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={TYPE_STROKE[type] ?? "rgba(255,255,255,0.5)"} strokeWidth="2">
...
export default async function DailyPulsePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div>
      ...
      {dates.length === 0 ? (
        <div className="border border-dashed rounded-xl p-12 text-center text-sm text-white/40" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
          {isEs ? "Contenido proximamente. Vuelve pronto." : "Content coming soon. Check back shortly."}
        </div>
      ) : (
        <>
          {/* Date nav */}
          <div className="flex items-center justify-between border rounded-xl px-5 py-3 mb-4" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
            <span className="text-sm text-white/25 flex items-center gap-1">
          ...
          {(podcast || blog) && (
            <div className="border rounded-xl p-5 mb-5" style={{ backgroundColor: "rgba(232,82,42,0.06)", borderColor: "rgba(232,82,42,0.15)" }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-semibold text-white">{isEs ? "Resumen de Hoy" : "Today's Recap"}</h2>
              </div>
              {podcast && (
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {(["podcast", "ai", "webpage", "youtube"] as const).map((type) => {
              ...
              return (
                <div key={type} className="border rounded-xl px-3 py-3 flex items-center gap-2" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
