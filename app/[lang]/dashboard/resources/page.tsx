          ...
          <form action={submitResource} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[var(--t3)] mb-1">{isEs ? "Nombre del recurso" : "Resource name"} *</label>
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[var(--t3)] mb-1">{isEs ? "Descripcion corta" : "Short description"}</label>
            ...
            <a key={r.id} href={r.url || "#"} target={r.url ? "_blank" : undefined} rel={r.url ? "noopener noreferrer" : undefined}
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <p className="text-sm font-semibold text-[var(--t1)]">{isEs && r.title_es ? r.title_es : r.title}</p>
                  {r.is_elite && <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-400/10 text-amber-400">Elite</span>}
                </div>
                <p className="text-xs text-[var(--t3)] leading-snug">{isEs && r.description_es ? r.description_es : r.description}</p>
              </div>
