    ...
    <div>
      ...
      {perks.length > 0 ? (
        <div className="space-y-3">
          {perks.map((perk) => (
            <div key={perk.id} className="border rounded-xl p-4 flex items-center gap-4 hover:border-opc-orange/30 transition-all" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  <span className="font-semibold text-[var(--t1)] text-sm">{perk.brand}</span>
                ...
                <p className="text-xs text-[var(--t3)]">{isEs && perk.offer_es ? perk.offer_es : perk.offer}</p>
                <p className="text-[10px] text-[var(--t4)] mt-0.5">{perk.claim_count} {isEs ? "reclamados" : "claimed"}</p>
              </div>
