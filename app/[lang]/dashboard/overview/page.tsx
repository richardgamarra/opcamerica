...
export default async function OverviewPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-400 mb-1 capitalize">{today}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <textarea
            ...
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 placeholder:text-gray-300 resize-none focus:outline-none focus:border-gray-300"
          />
      ...
      <div className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-5 flex items-center gap-4 text-gray-300">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
            <rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/>
            <line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
          </svg>
          <div>
            <p className="text-sm font-medium text-gray-400">{isEs ? "Sin eventos guardados" : "No saved events yet"}</p>
            <p className="text-xs text-gray-300">{isEs ? "Guarda cualquier evento en la pestaña Eventos." : "Tap the bookmark on any event in the Events tab to see it here."}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
        {quickActions.map((action) => (
          <Link key={action.href} href={action.href}
            className="bg-white rounded-xl border border-gray-100 p-4 flex items-start gap-3 hover:border-gray-200 hover:shadow-sm transition-all group">
            <span className="text-xl flex-shrink-0 mt-0.5">{action.icon}</span>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-gray-800 group-hover:text-opc-orange transition-colors">{action.title}</p>
              <p className="text-xs text-gray-400 mt-0.5 leading-snug">{action.sub}</p>
            </div>
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-center justify-between gap-4">
        ...
        <Link href={`/${lang}/join-elite`}
          className="text-sm font-semibold text-amber-700 border border-amber-300 px-4 py-1.5 rounded-lg hover:bg-amber-100 transition-colors whitespace-nowrap">
