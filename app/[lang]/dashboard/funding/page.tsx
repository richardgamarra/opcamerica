    ...
    <div>
        ...
        <div className="border rounded-xl p-5 mb-6" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
          ...
          <form action={submitFunding} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-white/40 mb-1">{isEs ? "Nombre del programa" : "Program name"} *</label>
              ...
              <div>
                ...
                <select name="type" className="w-full rounded-lg px-3 py-2 text-sm text-white/70 focus:outline-none focus:border-opc-orange transition-colors"
                  style={{ backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              <div>
                <label className="block text-xs font-semibold text-white/40 mb-1">{isEs ? "Pais" : "Country"} *</label>
                <input name="country" type="text" required placeholder="Mexico"
                  className="w-full rounded-lg px-3 py-2 text-sm text-white/70 placeholder:text-white/20 focus:outline-none focus:border-opc-orange transition-colors"
                  style={{ backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/40 mb-1">{isEs ? "Monto" : "Amount"}</label>
                <input name="amount" type="text" placeholder="$50,000"
                  className="w-full rounded-lg px-3 py-2 text-sm text-white/70 placeholder:text-white/20 focus:outline-none focus:border-opc-orange transition-colors"
                  style={{ backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/40 mb-1">{isEs ? "Fecha limite" : "Deadline"}</label>
      ...
      {programs.length > 0 ? (
        <div className="space-y-3">
          {programs.map((p) => (
            <div key={p.id} className="border rounded-xl p-4 hover:border-opc-orange/30 transition-all" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {p.country && <span className="text-[10px] text-white/40 border px-2 py-0.5 rounded" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>{p.country}</span>}
                  </div>
                  <div className="flex items-center gap-4 text-xs text-white/40">
                    {p.amount && <span className="font-semibold text-white/70">{p.amount}</span>}
                    {p.deadline && <span>{isEs ? "Cierre:" : "Deadline:"} <span className="font-medium text-white/55">{p.deadline}</span></span>}
                  </div>
                  {p.tags && p.tags.length > 0 && (
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {p.tags.map((t: string) => (
                        <span key={t} className="text-[10px] text-white/40 border px-2 py-0.5 rounded" style={{ backgroundColor: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.08)" }}>{t}</span>
                      ))}
                    </div>
                  )}
                </div>
